...
public class OrderController {
    ...
    ApiResponse<OrderResponse> createOrder(@RequestBody OrderRequest orderRequest) {
        OrderResponse orderResponse = orderService.createOrder(orderRequest.getCouponCode(), orderRequest.getAddressId(), orderRequest.getPaymentMethod());

        return ApiResponse.<OrderResponse>builder().code(200).result(orderResponse).build();
    }

    @GetMapping("/orders/search")
    public Page<OrderPageResponse> searchOrders(@RequestParam(required = false) String status, @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime startDate, @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime endDate, @RequestParam(required = false) String fullName, @RequestParam(required = false) String phoneNumber, @RequestParam(required = false) String addressDetail, @RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size) {

        return orderService.searchOrders(status, startDate, endDate, fullName, phoneNumber, addressDetail, page, size);
    }

    @GetMapping("/orders")
    public ResponseEntity<Page<OrderPageResponse>> getOrdersByStatus(@RequestParam String status, @RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "2") int size) {
    ...
    ApiResponse<OrderCancelResponse> cancelOrder(@PathVariable Long orderId) {
        return ApiResponse.<OrderCancelResponse>builder().result(orderService.cancelOrder(orderId)).code(200).build();
    }

    @PutMapping("/{orderId}/status")
    ApiResponse<OrderResponse> updateOrderStatus(@PathVariable Long orderId, @RequestParam String status) {
        return ApiResponse.<OrderResponse>builder().code(200).result(orderService.updateOrderStatus(orderId, status)).build();
    }
    ...
    ApiResponse<List<OrderResponse>> getCompletedOrders() {
        return ApiResponse.<List<OrderResponse>>builder().code(200).result(orderService.getCompletedOrdersByUser()).build();
    }

    @GetMapping("not-completed")
    ApiResponse<List<OrderResponse>> getNotCompletedOrders() {
        return ApiResponse.<List<OrderResponse>>builder().code(200).result(orderService.getNotCompletedOrdersByUser()).build();
    }
